...
// gen pass: 1
// console.log(crypto.createHmac('sha256', '68ac68').update('123').digest('hex'));
...
function checkAuthenticated(req, res, next) {
  ...
  var token =
    (req.body && req.body.token ? req.body.token : null) ||
  ...
  if (token) {
    ...
    return jwt.verify(
      token,
      _config.cisco.auth.secret,
      function (err, decoded) {
        if (err) {
          // log.info(MSG_LOG.AUTH_API.FAIL);
          const err = new ResError(
            ERR_401.code,
